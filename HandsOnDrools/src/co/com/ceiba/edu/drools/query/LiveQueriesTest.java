...
public class LiveQueriesTest {
    ...
    public void serverCpuUsageQuery() {
        ...
        LiveQuery query = ksession.openLiveQuery("serverCpuUsage", new Object[] { 200 }, listener);

        assertEquals(0, listener.getCurrentServers().size());
    ...
    public void serverCpuUsageGlazedListQuery() {
        ...
        Server winServer = new Server("winServer", 1, 4096, 2048, 25);
        FactHandle winServerFactHandle = ksession.insert(winServer);

        Server ubuntuServer = new Server("ubuntuServer", 4, 512, 200, 70);
        FactHandle ubuntuServerFactHandle = ksession.insert(ubuntuServer);

        Server debianServer = new Server("debianServer", 1, 1024, 250, 10);
        ksession.insert(debianServer);

        GlazedListViewChangedEventListener listener = new GlazedListViewChangedEventListener();
        LiveQuery query = ksession.openLiveQuery("serverCpuUsage", new Object[] { 40 }, listener);

        SortedList<Row> serverSortedList = new SortedList(listener, new Comparator<Row>() {
